...
namespace M3D.Graphics.Ext3D
{
  public class Element3D : IElement
  {
    ...
    {
      this.parent = parent;
      this.child_list3D = new List<Element3D>();
    }

    public virtual void Render3D()
    {
      lock (this.child_list3D)
      {
        foreach (Element3D element3D in this.child_list3D)
        {
    ...
    public virtual void RemoveChildElement(Element3D child)
    {
      ...
      if (this.child_list3D == null)
        return;
      lock (this.child_list3D)
        this.child_list3D.Remove(child);
    }
