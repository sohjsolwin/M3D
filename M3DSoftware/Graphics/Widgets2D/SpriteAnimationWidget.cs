...
namespace M3D.Graphics.Widgets2D
{
  public class SpriteAnimationWidget : ImageWidget
  {
    ...
    public void Init(GUIHost host, string texture, float normal_u0, float normal_v0, float normal_u1, float normal_v1, int num_columns, int num_rows, int num_frames, uint frame_time)
    {
      ...
      var num1 = 0;
      var num2 = 0;
      var num3 = (int)(normal_u1 - (double)normal_u0) / num_columns + 1;
      var num4 = (int)(normal_v1 - (double)normal_v0) / num_rows + 1;
      var num5 = 0.0f;
      var num6 = 0.0f;
      float num7;
      float num8;
      ...
      {
        num5 = 1f / Sprite.texture_width_pixels;
        num6 = 1f / Sprite.texture_height_pixels;
        num7 = 1f / Sprite.texture_width_pixels;
        num8 = 1f / Sprite.texture_height_pixels;
      }
      else
      {
        num7 = 1f / Sprite.texture_width_pixels;
      ...
      for (var index = 0; index < num_frames; ++index)
      {
        ...
        frames[index].x1 = 1f;
        frames[index].y1 = 1f;
        frames[index].u0 = (normal_u0 + num1 * num3) * num7;
        frames[index].v0 = (normal_v0 + num2 * num4) * num8;
        frames[index].u1 = (normal_u0 + (num1 + 1) * num3) * num7 + num5;
        frames[index].v1 = (normal_v0 + (num2 + 1) * num4) * num8 + num6;
        ++num1;
        if (num1 >= num_columns)
        {
          num1 = 0;
